...
class NetworkServer(
    ...
    private suspend fun runTcpServer(port: Int, startupComplete: CompletableDeferred<Unit>? = null) {
        try {
            ...
            serverSocket = aSocket(manager).tcp().bind("0.0.0.0", port = port) {
                // 启用 TCP keep-alive，更快检测死连接
                keepAlive = true
                // 禁用 Nagle 算法，降低音频传输延迟
                noDelay = true
                // 设置 socket 超时（影响 accept 后的读写）
                socketTimeout = Constants.TCP_CONNECTION_TIMEOUT_MS
            }
            ...
            while (currentCoroutineContext().isActive) {
                ...
                // 优化：设置连接 socket 选项
                socket.apply {
                    // 这些选项在 Ktor 中可能不直接支持，依赖 bind 时的配置
                }
    ...
    private suspend fun handleConnection(
        ...
        try {
            ...
            try {
                // 优化：添加读取超时
                withTimeoutOrNull(Constants.TCP_CONNECTION_TIMEOUT_MS) {
                    input.readFully(magicBytes)
                } ?: run {
                    Logger.w("NetworkServer", "Protocol detection timeout after ${Constants.TCP_CONNECTION_TIMEOUT_MS}ms")
                    closeAction()
                    return
                }
